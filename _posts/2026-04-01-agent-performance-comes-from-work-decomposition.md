...
에이전트 얘기를 하면 대화가 금방 모델 비교로 흐른다. 뭐가 더 똑똑한지, 긴 문맥을 누가 더 잘 버티는지, 코드를 누가 더 잘 쓰는지부터 따지게 된다. 당연한 반응이다. 나도 처음엔 거의 그렇게 봤다.

그런데 실제로 며칠만 굴려 보면 좀 다른 장면이 나온다. 같은 모델인데 어떤 날은 일이 술술 끝나고, 어떤 날은 비슷한 요청도 중간에서 흐트러진다. 이럴 때는 모델 점수표보다 **작업을 어떻게 나눴는지**, **세션을 어떻게 분리했는지**, **병렬 작업을 어떤 규칙으로 돌렸는지**가 더 크게 작용할 때가 많았다.

최근 GeekNews에서도 Claude Code의 숨겨진 기능들, 세션 포크, 병렬 워크트리, 하네스 설계 같은 이야기가 같이 보였다. 나는 그걸 보면서 다시 같은 생각을 했다. 결국 체감 성능 차이는 모델 그 자체보다 **작업 분해와 세션 구조**에서 많이 난다는 점이다.
...
결국 에이전트의 체감 성능은 “얼마나 똑똑한가”만이 아니라 **얼마나 안 꼬이게 만들어 놨는가**에도 크게 좌우된다.
...
그래서 병렬 실행은 그냥 "더 빠르게"의 문제가 아니다. 실제로는 **무엇을 동시에 돌려도 되는지 구분하는 문제**에 더 가깝다.
...
재밌는 건 이 중 대부분이 모델 스펙보다 **운영 구조의 품질**에 더 가깝다는 점이다.
